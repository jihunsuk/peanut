...
class Dialogue:
    ...
    def next_batch(self, batch_size):
        ...
        for i in range(0, len(batch_set) - 1):
            ...
            if len(batch_set[i]) > 70:
                enc_input = batch_set[i][0:70]
            dec_input = batch_set[i + 1]
            if len(batch_set[i + 1]) > 69:
                dec_input = batch_set[i + 1][0:69]

            enc, dec, tar = self.transform(enc_input, dec_input, 70, 70)
