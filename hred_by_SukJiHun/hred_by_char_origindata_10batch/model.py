...
class Hred:
    ...
    def __init__(self, vocab_size, training_mode, iterNumber, n_hidden=128, n_layers=3):
        ...
        self.enc_input_idx = tf.placeholder(tf.int32, [None, None, 70], name='enc_input_idx')
        self.dec_input_idx = tf.placeholder(tf.int32, [None, None, 70], name='dec_input_idx')
        self.targets = tf.placeholder(tf.int32, [None, None, 70], name='targets')
    ...
    def build_ops(self, outputs, targets):
        ...
        with tf.variable_scope('ops', reuse=tf.AUTO_REUSE):
            for i in range(self.iterNumber):
                ...
                f = tf.fill([9, tf.subtract(70, self.dec_max_length[i]), 1719], 0.0)
                new_logits = tf.stack(tf.concat([logits, f], axis=1))
