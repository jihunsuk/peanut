...
def train(dialog, batch_size=10, epoch=100):
    ...
    with tf.Session() as sess:
        ...
        for step in range(total_batch * epoch):
            ...
            b = np.max(dec_lengths, 1)
            context_size = len(enc_input)
            _, loss = sess.run([model.train_op, model.cost],
                               feed_dict={model.enc_input_idx: enc_inputs, model.dec_input_idx: dec_inputs, model.enc_length: enc_lengths,model.dec_length: dec_lengths, model.targets: targets, model.dec_max_length: b, model.context_size:context_size})
